            ...
            for (const file of acceptedFiles) {
                ...
                const fileContent = await new Promise<string>((resolve) => {
                    ...
                    if (file.type.startsWith('image/')) {
                        ...
                        // For now, we'll try to treat PDF as text if possible or just note we need a converter
                        // Simplest for MVP: treat everything as data URL and let server handle if possible
                        reader.readAsDataURL(file);
                    } else {
                    ...
                    {extractedLeads.length > 0 && !isProcessing && !isImporting && !result && (
                        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                            ...
                            <div className="pt-6 border-t border-border flex gap-4">
                                <Button className="flex-1 h-16 rounded-3xl bg-foreground text-background font-black uppercase text-xs tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl" onClick={handleConfirmImport}>
                                    Confirmar Importação de {extractedLeads.length} Leads
                                </Button>
                            </div>
                        </div>
                    )}
                    ...
                    {isImporting && (
                        <div className="py-20 flex flex-col items-center justify-center gap-8">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-muted-foreground font-medium mt-1">Verificando quarentena e duplicados.</p>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="py-10 space-y-8 animate-in zoom-in duration-500">
                            <div className="flex flex-col items-center gap-6">
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-muted-foreground font-medium mt-1">Relatório da operação:</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="p-6 rounded-[32px] bg-hot/5 border border-hot/20 text-center">
                                    <p className="text-3xl font-black text-hot">{result.errors}</p>
                                    <p className="text-[10px] font-black text-hot/60 uppercase tracking-widest mt-1">Erros</p>
                                </div>
                            </div>
