...
export default function EditPropertyPage() {
        ...
        async function load() {
            ...
            if (data) {
                form.reset({
                    ...
                    standard: (data.standard as any) || "medio",
                    targetAudience: data.targetAudience || [],
                    photos: data.photos || [],
                    minhaCasaMinhaVida: !!data.minhaCasaMinhaVida,
                    allowsFinancing: !!data.allowsFinancing,
                    isCondo: !!data.isCondo,
                    downPayment: data.downPayment || "",
                    condoFee: data.condoFee || "",
                });
            } else {
                toast.error("Imóvel não encontrado", { duration: 3000 });
                router.push("/imoveis");
            }
            setIsLoading(false);
        }
    ...
    async function onSubmit(values: PropertyFormValues) {
        setIsSaving(true);
        try {
            const result = await updateProperty(id as string, {
                ...values,
                ...
                <div className="bg-card border border-border/50 rounded-[28px] sm:rounded-[32px] shadow-2xl overflow-hidden relative">
                    ...
                    <div className="px-5 sm:px-8 py-4 sm:py-6 border-b border-border/50 flex items-center justify-between bg-muted/5">
                        <div className="space-y-0.5">
                            <h1 className="text-lg sm:text-xl font-black text-foreground uppercase tracking-tight">Editar Imóvel</h1>
                            <p className="text-[9px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-60">Atualização de Portfólio</p>
                        </div>
                        <Link href="/imoveis">
