...
import { getLaunchById, updateLaunch } from "@/lib/actions/launches";
...
export default function EditLaunchPage() {
    ...
    useEffect(() => {
        async function load() {
            if (!id) return;
            const data = await getLaunchById(id as string);
            if (data) {
                form.reset({
                    name: data.name || "",
            ...
            <div className="max-w-3xl mx-auto">
                <div className="bg-card border border-border rounded-[40px] shadow-2xl overflow-hidden relative">
                    ...
                    <div className="px-6 sm:px-10 py-6 sm:py-8 border-b border-border/40 flex items-center justify-between bg-muted/5 backdrop-blur-sm">
                        <div>
                            <h1 className="text-xl sm:text-2xl font-black text-foreground uppercase tracking-tight">Editar Empreendimento</h1>
                            <p className="text-[10px] sm:text-sm text-muted-foreground mt-1 font-bold uppercase tracking-widest opacity-60">Atualize o lançamento e suas plantas</p>
                        </div>
